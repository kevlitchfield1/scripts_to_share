...
genomeFreq <- function(data,thres.gain,thres.loss,pos.unit,layout,...){
  ...
  for(t in 1:nT){
    ...
    write.table(out_amp,"./CPI1000_analysis/CN_analysis/non_resp_amp_freqs.txt",quote=F,sep="\t")
    out_del<-cbind(xleft,freq.del)
    write.table(out_del,"./CPI1000_analysis/CN_analysis/non_resp_del_freqs.txt",quote=F,sep="\t")
    lines(xleft,freq.amp, type = "l", lty = 1, col = "red")
    lines(xleft,-freq.del, type = "l", lty = 1, col = "red")
    #rect(xleft=xleft,ybottom=0,xright=xright,ytop=freq.amp,col=op$col.gain,border=op$col.gain)
    #rect(xleft=xleft,ybottom=0,xright=xright,ytop=-freq.desl,col=op$col.loss,border=op$col.loss)
